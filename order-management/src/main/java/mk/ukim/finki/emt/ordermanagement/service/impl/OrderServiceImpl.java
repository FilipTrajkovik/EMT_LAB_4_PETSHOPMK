...
import mk.ukim.finki.emt.ordermanagement.domain.exceptions.OrderedDateNotValid;
...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderId placeOrder(OrderForm orderForm) {
        if (orderForm.getOrderedOn().isAfter(LocalDateTime.now()))
            throw new OrderedDateNotValid();
        ...
        newOrder.getOrderItemList().forEach(item->domainEventPublisher.publish(new OrderItemCreated(item.getProductId().getId(),item.getOrderItemQuantity())));
